...
public class BibTexServlet extends HttpServlet {
	...
	private BibTeXEntry bibEntryConverter(AnnotatedBiblographyEntry abe) {
		...
		switch (abe.getPublicationType().getPublicationTypeID()) {
			case 1: // Book
				bte = new BibTeXEntry(BibTeXEntry.TYPE_BOOK, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
				checkAndAdd(bte, BibTeXEntry.KEY_BOOKTITLE, abe.getTitleFull());
				break;
			
			case 3: // PhD 
				bte = new BibTeXEntry(BibTeXEntry.TYPE_PHDTHESIS, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
				checkAndAdd(bte, BibTeXEntry.KEY_TITLE, abe.getTitleFull());
				break;
			
			case 4: // Incollection
				bte = new BibTeXEntry(BibTeXEntry.TYPE_INCOLLECTION, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
				checkAndAdd(bte, BibTeXEntry.KEY_BOOKTITLE, abe.getParentTitleORG());
				checkAndAdd(bte, BibTeXEntry.KEY_TITLE, abe.getTitleFull());
				break;
			
			case 5: // Book section
				bte = new BibTeXEntry(BibTeXEntry.TYPE_INBOOK, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
				checkAndAdd(bte, BibTeXEntry.KEY_BOOKTITLE, abe.getParentTitleORG());
				checkAndAdd(bte, BibTeXEntry.KEY_CHAPTER, abe.getTitleFull());
				break;
			
			case 7: // Electronic
				bte = new BibTeXEntry(BibTeXEntry.TYPE_MISC, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
				checkAndAdd(bte, BibTeXEntry.KEY_TITLE, abe.getTitleFull());
				break;
			
			case 8: // Journal Article
				bte = new BibTeXEntry(BibTeXEntry.TYPE_ARTICLE, new Key(abe.getBibtexKey().isEmpty() ? abe.getUniqueID() : abe.getBibtexKey()));
